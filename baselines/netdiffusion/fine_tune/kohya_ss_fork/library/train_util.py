...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def make_buckets(self):
        ...
        if not self.enable_bucket:
            print("Step 3: Initializing BucketManager with predefined resolution.")
            self.bucket_manager = BucketManager(False, (self.width, self.height), None, None, None)
            self.bucket_manager.set_predefined_resos([(self.width, self.height)])  # ひとつの固定サイズbucketのみ
            for image_info in tqdm(self.image_data.values(), desc="Selecting predefined buckets"):
                image_width, image_height = image_info.image_size
                image_info.bucket_reso, image_info.resized_size, _ = self.bucket_manager.select_bucket(image_width, image_height)

        print("Step 4: Adding images to buckets.")
        for image_info in tqdm(self.image_data.values(), desc="Adding images to buckets"):
            # for _ in range(image_info.num_repeats):
            self.bucket_manager.add_image(image_info.bucket_reso, image_info.image_key)

        print("Step 5: Creating index for dataset shuffling.")
        self.buckets_indices: List(BucketBatchIndex) = []
        total_buckets = len(self.bucket_manager.buckets)
        print(f"Total number of buckets: {total_buckets}")
        for bucket_index, bucket in enumerate(tqdm(self.bucket_manager.buckets, desc="Creating dataset indices")):
            batch_count = int(math.ceil(len(bucket) / self.batch_size))
            print(f"Bucket {bucket_index}: batch_count={batch_count}, bucket_size={len(bucket)}")
            #       Bucket 0:              batch_count=9169020025,    bucket_size=9169020025
            for batch_index in range(batch_count):
                self.buckets_indices.append(BucketBatchIndex(bucket_index, self.batch_size, batch_index))
                if batch_index % 10000 == 0:  # Adjust the modulus value as needed to reduce verbosity
                    print(f"Processed {batch_index} batches for bucket {bucket_index}")

        print("Step 6: Shuffling buckets.")
        self.shuffle_buckets()
    ...
    def __init__(
        ...
        for subset in subsets:
            ...
            if subset.is_reg:
                ...
                num_reg_images += len(img_paths)
            else:
                # num_train_images += subset.num_repeats * len(img_paths)
                num_train_images += len(img_paths)
    ...
    def __init__(
        ...
        for subset in subsets:
            ...
            for image_key, img_md in metadata.items():
                ...
                image_info = ImageInfo(image_key, 1, caption, False, abs_path)
