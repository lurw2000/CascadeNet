...
org_dir = '../../../result/netdiffusion/color_processed_generated_imgs/'
files = [f for f in os.listdir(org_dir) if f.endswith('.png')]

for i in tqdm(files, desc="Processing images"):
    if 'png' in i:
        input_file = org_dir + i
        reverse_df = png_to_dataframe(input_file)
        output_file = '../../../result/netdiffusion/generated_nprint/' + i.replace('png','nprint')
