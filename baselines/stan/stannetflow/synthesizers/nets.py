...
class MixtureDiagNormalNetwork(nn.Module):
    ...
    #     mean = torch.stack(mean.split(mean.shape[1] // self.n_components, 1))
    #     sd = torch.stack(sd.split(sd.shape[1] // self.n_components, 1))
    #     sd = torch.exp(sd).clamp(min=1e-6, max=1e6)
    #     return Normal(mean.transpose(0, 1), sd.transpose(0, 1))

    # def forward(self, x):
    #     print("Input to forward pass:", x)
        
    #     params = self.network(x)
        
    #     # Debugging: Check if params contain any nan or inf values
    #     if torch.isnan(params).any() or torch.isinf(params).any():
    #         print("Invalid 'params' detected in forward pass:", params)
    #         raise ValueError("Invalid 'params' detected during forward pass.")

    #     # Split params into mean and sd
    #     mean, sd = torch.split(params, params.shape[1] // 2, dim=1)
        
    #     # Debugging: Check mean and sd before further processing
    #     print("Initial mean:", mean)
    #     print("Initial sd:", sd)
        
    #     if torch.isnan(mean).any() or torch.isinf(mean).any():
    #         print("Error: 'nan' or 'inf' values detected in 'mean'.")
    #         raise ValueError("Invalid 'mean' detected during forward pass.")

    #     mean = torch.stack(mean.split(mean.shape[1] // self.n_components, 1))
    #     sd = torch.stack(sd.split(sd.shape[1] // self.n_components, 1))

    #     # Apply the exponential to the standard deviation and clamp it to avoid zeros or inf
    #     sd = torch.exp(sd).clamp(min=1e-6, max=1e6)

    #     if torch.isnan(sd).any() or torch.isinf(sd).any():
    #         print("Error: 'nan' or 'inf' values detected in 'sd'. Clamping to valid range.")
    #         sd = torch.nan_to_num(sd, nan=1e-6, posinf=1e6, neginf=1e6)
    #         if torch.isnan(sd).any() or torch.isinf(sd).any():
    #             raise ValueError("Invalid 'sd' detected during forward pass.")
    ...
    def forward(self, x):
        try:
            ...
            #     raise ValueError("NaN detected in network parameters")
            if torch.isnan(params).any():
                print("NaN values detected in the following parameters:", params)
                raise ValueError("NaN detected in network parameters")

            params = torch.clamp(params, min=-1e6, max=1e6)  # Clamp logits

            epsilon = 1e-6
            params = params + epsilon  # Adding epsilon to logits
        ...
        sd = torch.nan_to_num(sd, nan=1e-6, posinf=1e6, neginf=1e-6)

        if torch.isnan(mean).any() or torch.isinf(mean).any():
            print("Error: 'nan' or 'inf' values detected in 'mean'.")
            print("Mean values causing error:", mean)
            raise ValueError("Invalid 'mean' detected during forward pass.")
...
class CategoricalNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        # Clamping the logits to avoid extreme values
        params = torch.clamp(params, min=-1e6, max=1e6)
        
        # Check for NaN or Inf values in the logits
        params = torch.nan_to_num(params, nan=0.0, posinf=1e6, neginf=-1e6)
        if torch.isnan(params).any() or torch.isinf(params).any():
            print("Error: 'nan' or 'inf' values detected in 'logits'.")
            print("Logits causing error:", params)
            raise ValueError("Invalid 'logits' detected during forward pass.")
