...
class PCAP:
    ...
    @timing
    def _flow2features(self, feat_type='IAT', *, dim=None, sampling_rate=None, fft=False, header=False):
        ...
        if feat_type in ['IAT', 'FFT-IAT']:
            ...
            if sampling_rate is None:
                ...
                self.sampling_rate = _get_split_interval(flow_durations, q_interval=0.3)
            else:
    ...
    def flow2features(self, feat_type='IAT', *, dim=None, sampling_rate=None, fft=False, header=False):
        """Extract features from each flow according to feat_type, fft and header.
        ...
        -------
            self
        """
        _, tot_time = self._flow2features(feat_type, dim=dim, sampling_rate=sampling_rate, fft=fft, header=header)
